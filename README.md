...
|-------|----------|-----------|----------------|
| POST | `/api/auth/login` | Жүйеге кіру | қажет емес     |

### Продукттер API (`/api/products`)

| Метод | Эндпоинт | Сипаттама | Аутентификация |
|-------|----------|-----------|----------------|
| GET | `/api/products` | Барлық өнімдерді алу (keyword параметрімен фильтрлеу) | қажет емес       |
| GET | `/api/products/{id}` | Өнімді ID бойынша алу | қажет емес         |
...
| Метод | Эндпоинт | Сипаттама | Аутентификация |
|-------|----------|-----------|----------------|
| POST | `/api/customers/register` | Тұтынушыны тіркеу (Customer + UserAccount құру) | қажет емес |
